...
export default {
  ...
  methods: {
    ...
    cancelAuthUser(row) {
      ...
      this.$modal.confirm('Confirm cancelling user "' + row.userName + '"roles?').then(function() {
        return authUserCancel({ userId: row.userId, roleId: roleId });
      }).then(() => {
        this.getList();
        this.$modal.msgSuccess("Remove roles successfully!");
      }).catch(() => {});
    },
    /** 批量取消授权按钮操作 */
    cancelAuthUserAll(row) {
      ...
      this.$modal.confirm('Confirm cancelling user roles?').then(function() {
